...
async fn test_stress_concurrent_peers_connection_load() -> anyhow::Result<()> {
    ...
    let cfg = MeshConfig {
        ...
        max_events_per_second_per_peer: 100,
        ..MeshConfig::default()
    };
    ...
    for i in 0..peer_count {
        ...
        let handle = tokio::spawn(async move {
            if let Ok((mut ws, _)) = connect_async(&url).await {
                let evt = format!(
                    r#"["EVENT",{{"id":"stress_evt_{i}","content":"stress test payload"}}]#"#
                );
                let _ = ws.send(Message::Text(evt)).await;
    ...
    assert!(
        metrics_resp.is_ok() || metrics_resp.is_err(),
        "Server processed concurrent connections without deadlock"
    );
